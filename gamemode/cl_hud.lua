...
usermessage.Hook( "UBC", UpBootCounter );
...
usermessage.Hook( "DBC", DownBootCounter );
...
usermessage.Hook( "RBC", ResetBootCounter );
...
function DrawFade()
	...
	local TimeSinceFade = CurTime() - FadeStart;
	
	if( TimeSinceFade > 0 and TimeSinceFade <= FadeTimeIn ) then
		
		local mul = ( CurTime() - FadeStart ) / FadeTimeIn;
		local col = FadeColor;
	...
	elseif( TimeSinceFade > FadeTimeIn and TimeSinceFade <= FadeTimeIn + FadeTimeOut ) then
		
		local mul = ( CurTime() - FadeStart ) / FadeTimeOut;
		local col = FadeColor;
...
function DrawEndgame()
	...
	if( GameOver ) then
		...
		local mul = math.Clamp( ( CurTime() - FadeStart ) / FadeTimeOut + 1, 0, 1 );
